    ...
    public Target(Tile new_center, float[,] area)
    {
        ...
        for (int x = 0; x < area.GetLength(0); x++)
        {
            for (int y = 0; y < area.GetLength(1); y++)
            {
                if (area[x,y] != 0)
                {
                    //Transform target = character.controller.Get_Curr_Scenario().tile_grid.getTile(startX + x, startY + y);
                    Tile target = Game_Controller.Get_Curr_Scenario().Get_Tile(startX + x, startY + y).GetComponent<Tile>();
                    if (target != null)
                    {
